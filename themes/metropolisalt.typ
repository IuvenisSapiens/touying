  ...
  let body = {
    ...
    block(
      ...
      inset: 1em,
      {
        components.left-and-right(
          {
            text(size: 2em, text(weight: "extrabold", info.title))
            if info.subtitle != none {
              linebreak()
              text(size: 1.5em, info.subtitle, weight: "medium", baseline: 0.25em)
            }
          },
          text(2.5em, utils.call-or-display(self, info.logo)),
        )
...
/// Touying metropolisalt theme.
///
/// Example:
///
/// ```typst
/// #show: metropolisalt-theme.with(aspect-ratio: "16-9", config-colors(primary: blue))`
/// ```
///
/// Consider using:
///
...
#let metropolisalt-theme(
  ...
  footer: none,
  footer-right: context if utils.slide-counter.get().first() <= utils.last-slide-counter.get().first() {
    (
      text(fill: rgb("#eb811b"), utils.slide-counter.display())
        + text(fill: rgb("#d6c6b7"), [~/~])
        + text(fill: rgb("#d6c6b7"), utils.last-slide-number)
    )
  } else {
    (
      text(fill: rgb("#eb811b"), context numbering(
        "I",
        utils.slide-counter.get().first() - utils.last-slide-counter.get().first(),
      ))
        + text(fill: rgb("#d6c6b7"), [~/~])
        + text(fill: rgb("#d6c6b7"), context numbering(
          "I",
          utils.slide-counter.final().first() - utils.last-slide-counter.get().first(),
        ))
    )
  },
